...
class TipoToken:
    #######
    #Falta cabar de completar depois com as palavras reservdadas
    #
    #
    #
    #######
    ...
    BOOLEAN = ('BOOLEAN', 'VLogico')
    PROGRAMA = ('Programa', 'Programa')
    REAL = ('Real', 'TReal')
    INTEIRO = ('Inteiro', 'TIntero')
    VARIAVEL = ('VAR', 'Variavel')
    CONDICAO = ('Condicao', 'Condicional')
    LACO = ('Laco', 'TLaco')
    READ = ('Read', 'Leitura')
    WRITE = ('Write', 'Leitura')
...
class Lexico:
    ...
    reservadas = {'programa': TipoToken.PROGRAMA, 'variaveis': TipoToken.VARIAVEL, 'inteiro': TipoToken.INTEIRO, 'real': TipoToken.REAL, 'logico': TipoToken.LOGICO,
                  'caractere': TipoToken.CADCARACTER, 'se': TipoToken.CONDICAO, 'senao': TipoToken.CONDICAO, 'enquanto': TipoToken.LACO,
                  'leia': TipoToken.READ, 'escreva': TipoToken.WRITE, 'falso': TipoToken.BOOLEAN , 'verdadeiro': TipoToken.BOOLEAN}
...
if __name__== "__main__":
   ...
   while(True):
       token = lex.getToken()
       print("token= %s , lexema= (%s), linha= %d" % (token.id, token.lexema, token.linha))
